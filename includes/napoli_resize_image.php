<?php
if ( !function_exists('napoli_resize_image') ) :
    /**
     * @author AZ-Theme
     * @param int $attach_id
     ...
     * @return array
     */
    function napoli_resize_image( $attach_id = null, $img_url = null, $width, $height, $crop = false, $place_hold = true )
    {
    	...
    	if ( ! empty( $actual_file_path ) && file_exists( $actual_file_path ) )
    	{
    		...
    		if ( $image_src[1] > $width || $image_src[2] > $height )
    		{
    			...
    			if ( file_exists( $cropped_img_path ) ) {
    				$cropped_img_url 	= str_replace( napoli_basename( $image_src[0] ), napoli_basename( $cropped_img_path ), $image_src[0] );
    			...
    			if ( $crop == false )
    			{
    				...
    				if ( file_exists( $resized_img_path ) ) {
    					$resized_img_url 	= str_replace( napoli_basename( $image_src[0] ), napoli_basename( $resized_img_path ), $image_src[0] );
    			...
    			$new_img 		= str_replace( napoli_basename( $image_src[0] ), napoli_basename( $new_img_path ), $image_src[0] );
