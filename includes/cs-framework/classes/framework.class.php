...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'napoli' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'napoli' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'napoli' ), 'updated' );
    }
    ...
    set_transient( 'napoli-transient', array( 'errors' => $add_errors, 'section_id' => $section_id ), $transient_time );
  ...
  public function admin_page() {
    ...
    $transient  = get_transient( 'napoli-transient' );
    ...
    echo '<div class="napoli cs-option-framework">';
      ...
      echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">'. __( 'Settings saved.', 'napoli' ) .'</span>' : '';

      submit_button( __( 'Save', 'napoli' ), 'primary cs-save', 'save', false, array( 'data-save' => __( 'Saving...', 'napoli' ) ) );
      submit_button( __( 'Restore', 'napoli' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );

      if( $this->settings['show_reset_all'] ) {
        submit_button( __( 'Reset All Options', 'napoli' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique .'[resetall]', false );
      }

      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. __( 'show all options', 'napoli' ) .'</a>' : '';
